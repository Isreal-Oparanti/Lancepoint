...
const Login = () => {
  ...
  const handleGoogleLogin = async (credentialResponse) => {
        ...
        try {
          ...
          const response = await axios.post("https://x-ploit-backend-4.onrender.com/api/google-login", {
            idToken,
          });
    
          const { user, token } = response.data; // Backend should return user info and your app's token
    
          // Save user in localStorage or Context
          localStorage.setItem("user", JSON.stringify(user));
          setAuth({ user });
    
          // Store the token in localStorage or httpOnly cookie if needed
          ...
          <div className="bg-opacity-10 mt-8 flex flex-col justify-center bg-secondary-dark py-6 mx-auto shadow rounded-lg px-4 w-full max-w-md relative border-2 border-stone-700">
            <form onSubmit={handleFormSubmit} className="flex flex-col justify-center space-y-4 w-full">
              ...
              <button
                type="submit"
                disabled={loading}
                className={`bg-purple-700 text-white py-2 px-4 w-full text-sm rounded-full hover:bg-purple-800 transition ${loading ? "opacity-50 cursor-not-allowed" : ""}`}
              >
...
//     try {
//       // Send idToken to your backend for verification and authentication
//       const response = await axios.post("https://your-backend.com/api/google-login", {
//         idToken,
//       });
